...
import calcButtonTemp from "../../../assets/img/calcbuttontemp.png";
import BaseModal from "../../../components/BaseModal";
import {
	MONSTER_TOUR_COMPLETED_EVENT,
	MONSTER_TOUR_OPEN_MODAL_EVENT,
} from "../MonsterTour";
...
const triggerButtonClass =
	"fixed bottom-6 right-6 z-40 transform-gpu transition-transform duration-150 ease-out hover:scale-[1.01]";
const inputLabelClass = "text-sm font-medium text-zinc-700 dark:text-zinc-300";
const inputFieldClass =
	"mt-1 w-full rounded-lg border border-black/10 bg-white px-3 py-2 text-sm dark:border-white/15 dark:bg-black";
...
function worldLevelMultiplier(charLevel: number, worldLevel: number): number {
	...
	if (worldDelta > 3) {
		...
		const bonusLevels = worldDelta - 3;
		return Math.min(5, 1 + bonusLevels * 0.1);
	}

	if (worldDelta < 0) {
		const penaltyLevels = Math.abs(worldDelta);
		return Math.max(0, 1 - penaltyLevels * 0.1);
	}
...
export default function MonsterBestModal({
	monsters,
}: {
	monsters: Monster[];
}) {
	// Modal UI state + calculator inputs.
	...
	// Compute score candidates only after users interact with core inputs.
	...
	// Pick one best card per category (AFK/Odin + Base/Job).
	...
	useEffect(() => {
		const onOpenForTour = () => setIsOpen(true);
		const onTourCompleted = () => setIsOpen(false);

		window.addEventListener(MONSTER_TOUR_OPEN_MODAL_EVENT, onOpenForTour);
		window.addEventListener(MONSTER_TOUR_COMPLETED_EVENT, onTourCompleted);
		return () => {
			window.removeEventListener(MONSTER_TOUR_OPEN_MODAL_EVENT, onOpenForTour);
			window.removeEventListener(MONSTER_TOUR_COMPLETED_EVENT, onTourCompleted);
		};
	}, []);
		...
		<>
			...
			>
				<div
					...
					data-tour="monster-calculator-inputs"
				>
					<div>
						<label className={inputLabelClass} htmlFor="calc-char-level">
							Your Level
						</label>
						<select
							id="calc-char-level"
					...
					<div>
						<label className={inputLabelClass} htmlFor="calc-world-level">
							World Level
						</label>
						<select
							id="calc-world-level"
					...
					<div>
						<label className={inputLabelClass} htmlFor="calc-party-size">
							Party Size
						</label>
						<select
							id="calc-party-size"
					...
					<div>
						<label className={inputLabelClass} htmlFor="calc-unique-classes">
							Unique Classes
						</label>
						<select
				...
				<div
					...
					data-tour="monster-calculator-results"
				>
					{cards.map((card) => {
